...
use super::{
    ...
    TStructIdentifier, TType, BINARY_BASIC_TYPE_FIXED_SIZE,
};
...
pub struct TBinaryShmipcOutputProtocol<'a> {
    pub(crate) trans: &'a mut [u8],
    pub(crate) index: usize,
}

impl<'a> TBinaryShmipcOutputProtocol<'a> {
    #[inline]
...
impl<'a> TLengthProtocol for TBinaryShmipcOutputProtocol<'a> {
    #[inline]
...
impl<'a> TOutputProtocol for TBinaryShmipcOutputProtocol<'a> {
...
pub struct TBinaryShmipcInputProtocol<'a> {
    pub(crate) trans: &'a mut Bytes,
    pub(crate) buf: &'a [u8],
    pub(crate) index: usize,
}

impl<'a> TBinaryShmipcInputProtocol<'a> {
    /// # Safety
    ///
...
impl<'a> TLengthProtocol for TBinaryShmipcInputProtocol<'a> {
    #[inline]
    fn message_begin_len(&mut self, identifier: &TMessageIdentifier) -> usize {
        self.i32_len(0) + self.faststr_len(&identifier.name) + self.i32_len(0)
    }
...
impl<'a> TInputProtocol for TBinaryShmipcInputProtocol<'a> {
    type Buf = Bytes;
